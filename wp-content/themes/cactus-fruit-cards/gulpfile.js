...
// This task takes SASS files in src and compiles them into CSS in assets
...
gulp.task('sass-compile', (done) => {
	return gulp
		...
		.pipe(gulp.dest((file) => file.base.replace(path.sep + 'src', path.sep + 'assets').replace(path.sep + 'scss', path.sep + 'css'))) // put the css files here.
		...
		.pipe(
			gulpFn(function (file) {
				if (file.path.indexOf('.css.map') === -1) {
					...
					console.log(file.path.replace(path.sep + 'src', path.sep + 'assets').replace(path.sep + 'scss', path.sep + 'css'));
				}
			})
		);
});
...
// into older, more broadly compatible (ES5) JavaScript files in assets
...
gulp.task('js-compile', (done) => {
	return (
		gulp
			...
			.pipe(gulp.dest((file) => file.base.replace(path.sep + 'src', path.sep + 'assets'))) // put the js files here.
			.pipe(
				gulpFn(function (file) {
					if (file.path.indexOf('.js.map') === -1) {
						console.log('JS generated: '.cyan);
						console.log(file.path.replace(path.sep + 'src', path.sep + 'assets'));
					}
				})
			)
	);
...
// This task takes all images dropped in src and recompresses them, and puts them in assets
const filesForImageCompression = ['src/img/**/*.*'];
gulp.task('image-compress', (done) => {
	let imgDest = 'assets/img/';
	return gulp
		.src(filesForImageCompression) // watch these files
		.pipe(plumber())
		.pipe(changed((file) => file.base.replace('/src', '/assets')))
		.pipe(imagemin())
		.pipe(gulp.dest((file) => file.base.replace('/src', '/assets'))) // put the image files here.
		.pipe(browserSync.stream()) // tell browsersync to send over the changes
		.pipe(
			gulpFn(function (file) {
				console.log('Image compressed and copied to: '.cyan);
				console.log(file.path.replace('/src', '/assets'));
			})
		);
});
...
gulp.task('build', gulp.parallel('image-compress', 'js-compile', 'sass-compile'));
...
gulp.task('default', gulp.series('welcome', 'start-browsersync', 'build', 'watch', 'version'));
...
gulp.task(
	'no-browser-sync',
	gulp.series('welcome', gulp.parallel('image-compress', 'js-compile', 'sass-compile'), 'version')
);